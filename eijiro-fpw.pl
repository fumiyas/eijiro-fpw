...
while (<>) {
	s/\r?\n$//;
	next if (/^$/);
	&error("Unexpected line in $ARGV ($_)") if (!s/^Å°//);
...
sub write_word_info
{
	my $katakana = "(?:\x83[\x40-\x96])";	# [É@-Éñ]
	my($info) = @_;

	$info =~ s/ÅyÅóÅz(?:$katakana|\x81[\x41\x5b])*//;
...
sub write_meaning
{
	...
	#
	if ($mean =~ s/ \/ Åyópó·(?:ÅE$sjis_any+(?:\-[0-9]+)?)?Åz(.*)$//) {
		$yorei = $1;
	}
